...
! K-Points
  ...
  Type :: oPwscfOut
    ...
    Real(kind=DoubleReal) :: totalForce
    Integer(kind=StandardInteger) :: atomCount
    Real(kind=DoubleReal), Dimension(1:3,1:3) :: crystalAxes
    Type(oPwscfCoords), Dimension(:), Allocatable :: atoms
  End Type oPwscfOut
! CONTROL
...
! Public Subroutines
  ...
  Public :: readPwscfOutput
  Public :: printPwscfOutput
...
!---------------------------------------------------------
  ...
  Subroutine readPwscfOutput(filePath, pwscfOut)
! Reads a PWscf output file into an oPwscfOut object
    Implicit None  ! Force declaration of all variables
! Vars:  Input/Output
    Character(*) :: filePath
    Type(oPwscfOut) :: pwscfOut
! Vars:  Private
    Integer(kind=StandardInteger) :: i, j, n, dataRows
    Character(len=255), Dimension(1:4096) :: pwscfOutputData
    Character(Len=64), Dimension(1:10) :: fieldArray
    Character(Len=24) :: tempStr
    Logical :: initialCoords, flag
    Call readFileRaw(filePath, pwscfOutputData, dataRows)
    pwscfOut%filePath = trim(filePath)
    i = 0
    initialCoords = .false.
    Do While (i.le.dataRows)
      i = i + 1
      If(pwscfOutputData(i)(1:25).eq."     number of atoms/cell")Then
        tempStr = pwscfOutputData(i)(34:46)
        pwscfOut%atomCount = StrToInt(tempStr)
        Call initPwscfOutCoords(pwscfOut)
      End If
      If(pwscfOutputData(i)(1:17).eq."!    total energy")Then
        tempStr = pwscfOutputData(i)(33:50)
        pwscfOut%totalEnergy = StrToDP(tempStr)
        pwscfOut%totalEnergy = UnitConvert(pwscfOut%totalEnergy,"RY","EV")
      End If
      If(pwscfOutputData(i)(1:16).eq."     Total force")Then
        tempStr = pwscfOutputData(i)(20:34)
        pwscfOut%totalForce = StrToDP(tempStr)
        pwscfOut%totalForce = UnitConvert(pwscfOut%totalForce,"RYBOHR","EVANG")
      End If
      If(pwscfOutputData(i)(1:22).eq."     lattice parameter")Then
        tempStr = pwscfOutputData(i)(34:46)
        pwscfOut%aLat = StrToDP(tempStr)
        pwscfOut%aLat = UnitConvert(pwscfOut%aLat,"BOHR","ANG")
      End If
      If(pwscfOutputData(i)(1:19).eq."     crystal axes: ")Then
        Do j=1,3
          i = i + 1
          tempStr = pwscfOutputData(i)(25:35)
          pwscfOut%crystalAxes(j,1) = StrToDP(tempStr)
          tempStr = pwscfOutputData(i)(36:46)
          pwscfOut%crystalAxes(j,2) = StrToDP(tempStr)
          tempStr = pwscfOutputData(i)(47:57)
          pwscfOut%crystalAxes(j,3) = StrToDP(tempStr)
        End Do
      End If


      If(pwscfOutputData(i)(1:17).eq."   Cartesian axes")Then
        If(.not.initialCoords)Then
          initialCoords = .true.
          flag = .true.
          Do while(flag)
            i = i + 1
            If(pwscfOutputData(i)(1:12).eq."     site n.")Then
              flag = .false.
            End If
          End Do
          i = i + 1
          ...
          Do while(flag)
            n = n + 1
            fieldArray(1) = pwscfOutputData(i)(11:24)
            fieldArray(2) = pwscfOutputData(i)(39:51)
            fieldArray(3) = pwscfOutputData(i)(52:63)
            fieldArray(4) = pwscfOutputData(i)(64:75)
            pwscfOut%atoms(n)%label = fieldArray(1)(1:16)
            pwscfOut%atoms(n)%xyz(1) = StrToDP(fieldArray(2))
            pwscfOut%atoms(n)%xyz(2) = StrToDP(fieldArray(3))
            pwscfOut%atoms(n)%xyz(3) = StrToDP(fieldArray(4))
            i = i + 1
            If(IsBlank(pwscfOutputData(i)))Then
              flag = .false.
            End If
          End Do
        End If
      End If
      If(pwscfOutputData(i)(1:27).eq."     Forces acting on atoms")Then
        flag = .true.
        Do while(flag)
          i = i + 1
          If(pwscfOutputData(i)(1:9).eq."     atom")Then
            flag = .false.
          End If
        End Do
        flag = .true.
        ...
        Do while(flag)
          n = n + 1
          fieldArray(1) = pwscfOutputData(i)(35:48)
          fieldArray(2) = pwscfOutputData(i)(49:62)
          fieldArray(3) = pwscfOutputData(i)(63:76)
          pwscfOut%atoms(n)%forces(1) = StrToDP(fieldArray(1))
          pwscfOut%atoms(n)%forces(2) = StrToDP(fieldArray(2))
          pwscfOut%atoms(n)%forces(3) = StrToDP(fieldArray(3))
          i = i + 1
          If(IsBlank(pwscfOutputData(i)))Then
            flag = .false.
          End If
  ...
  End Subroutine readPwscfOutput

  Subroutine initPwscfOutCoords(pwscfOut)
...
! Allocate
    If(pwscfOut%atomCount.lt.1)Then
      pwscfOut%atomCount = 1024
    End If
    If(Allocated(pwscfOut%atoms))Then
      Deallocate(pwscfOut%atoms)
    End If
    Allocate(pwscfOut%atoms(1:pwscfOut%atomCount))
! Zero
    Do i=1,pwscfOut%atomCount
      pwscfOut%atoms(i)%label = "                "
      Do j=1,3
        pwscfOut%atoms(i)%xyz(j) = 0.0D0
        pwscfOut%atoms(i)%forces(j) = 0.0D0
      End Do
    End Do
  End Subroutine initPwscfOutCoords


  Subroutine printPwscfOutput(pwscfOut)
! Reset data objects for a plot
    Implicit None  ! Force declaration of all variables
! Vars:  Input/Output
    Type(oPwscfOut) :: pwscfOut
! Vars:  Private
    ...
    print "(A24,F18.8,A3)",  "Scf Energy (total):     ",pwscfOut%totalEnergy," eV"
    print "(A24,F18.8,A3)",  "Scf Energy (per atom):  ",(pwscfOut%totalEnergy/pwscfOut%atomCount)," eV"
    print "(A24,F18.8,A7)",  "Total Force          :  ",pwscfOut%totalForce," eV/ang"
    print "(A24,I18)",    "Atom count:             ",pwscfOut%atomCount
    Do i=1,pwscfOut%atomCount
      print "(I8,A16,F14.8,F14.8,F14.8,F14.8,F14.8,F14.8)",&
      i,pwscfOut%atoms(i)%label,&
      pwscfOut%atoms(i)%xyz(1),pwscfOut%atoms(i)%xyz(2),pwscfOut%atoms(i)%xyz(3),&
      pwscfOut%atoms(i)%forces(1),pwscfOut%atoms(i)%forces(2),pwscfOut%atoms(i)%forces(3)
    End Do

  End Subroutine printPwscfOutput
