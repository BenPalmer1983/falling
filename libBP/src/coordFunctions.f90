...
! Private variables
  ...
  Function RdCoords (xVect, yVect, distortionIn) RESULT (rd)
! Transform coords with transformation matrix
    Implicit None  ! Force declaration of all variables
! Vars:  In
    Real(kind=DoubleReal), Dimension(1:3) :: xVect, yVect
    Real(kind=DoubleReal), Dimension(1:3,1:3), Optional :: distortionIn
! Vars:  Out
    Real(kind=DoubleReal) :: rd, xD, yD, zD
    Real(kind=DoubleReal), Dimension(1:3,1:3) :: distortion
! Optional arguments
    If(Present(distortionIn))Then
      distortion = distortionIn
      xD = xVect(1) - yVect(1)
      yD = xVect(2) - yVect(2)
      zD = xVect(3) - yVect(3)
      rd = sqrt((distortion(1,1)*xD+distortion(1,2)*yD+distortion(1,3)*zD)**2&
           +(distortion(2,1)*xD+distortion(2,2)*yD+distortion(2,3)*zD)**2&
           +(distortion(3,1)*xD+distortion(3,2)*yD+distortion(3,3)*zD)**2)
    Else
! Calculate
