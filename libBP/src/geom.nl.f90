...
!----------------------------------------------
  ...
  Subroutine rebuildNL(nl, rVerlet, cKey_NL_in)
! Make neighbour list for atoms
! The input coords and alat must be large enough so the same atom does not interact
! with a copy in a periodic cell surrounding the original
! e.g. if the rVerlet cutoff is 5, the alat must be greater than 5
...
!----------------------------------------------
!
! Arrays already allocated - just rebuilding data here
!
!----------------------------------------------
! Build NL
!----------------------------------------------
...
!
    Do cKey=1,size(nl,1)
      If(nl(cKey)%coordsLength.gt.0)Then  ! Check that there are coordinates
        If((cKey_NL.eq.0).or.(cKey_NL.eq.cKey))Then
!----------------------------------------------
! Load coords from coordsMD
    Do i=1,nl(cKey)%coordsLength
      Do j=1,3
        position(j) = nl(cKey)%coordsMD(i,j)
      End Do
      positionT = matmul(nl(cKey)%unitCellScaledInverse,position)  ! Inverse transformation back to 1x1x1 box
      Do j=1,3
        nl(cKey)%coords(i,j) = Modulus(positionT(j),1.0D0)  ! x/y/z coord, keep within a 1x1x1 box
      End Do
    End Do
!----------------------------------------------
! Init config specific variables
    rVerletSQ = nl(cKey)%rVerletSQ
    aLat = nl(cKey)%aLat
    nlKey = 0
!----------------------------------------------
! Init overkey count
...
! Refresh neighbour list using coodsMD without rebuilding entire list
!
!
    Implicit None   ! Force declaration of all variables
! Vars:  In/Out
    ...
    Integer(kind=StandardInteger), Optional :: cKey_NL_in
! Vars:  Private
    Integer(kind=StandardInteger) :: l, m, n, cKey, cKey_NL, ghostCellKey
    Real(kind=DoubleReal), Dimension(1:27,1:3) :: ghostCellShift
    ...
    Do l=-1,1
      Do m=-1,1
        Do n=-1,1
          ...
          position(3) = n
          positionT = MatMul(nl(cKey)%unitCellScaled,position)
          ghostCellShift(ghostCellKey,1) = positionT(1)
          ghostCellShift(ghostCellKey,2) = positionT(2)
          ghostCellShift(ghostCellKey,3) = positionT(3)
        End Do
      End Do
    End Do
...
! Ghost cell key
      ghostCellKey = (nl(cKey)%ghostCell(n,1)+2)+&
                     (nl(cKey)%subCellWidth+1)*(nl(cKey)%ghostCell(n,2)+1)+&
                     (nl(cKey)%subCellWidth+1)**2*(nl(cKey)%ghostCell(n,3)+1)
...
! atom positions
      ...
      xB = nl(ckey)%coordsMD(nl(ckey)%atomB_ID(n),1)+ghostCellShift(ghostCellKey,1)
      yB = nl(ckey)%coordsMD(nl(ckey)%atomB_ID(n),2)+ghostCellShift(ghostCellKey,2)
      zB = nl(ckey)%coordsMD(nl(ckey)%atomB_ID(n),3)+ghostCellShift(ghostCellKey,3)
! atom displacements
      xD = xA - xB
      yD = yA - yB
      zD = zA - zB
...
!----------------------------------------------
  ...
  Subroutine distortNL(nl, distortion, cKey_NL_in)
! DEFUNCT - needs replacing with DistortNL
!
...
! Vars:  In/Out
    Type(nlType), Dimension(:) :: nl
    Real(kind=DoubleReal), Dimension(1:3,1:3) :: distortion, volVec
    Integer(kind=StandardInteger), Optional :: cKey_NL_in
! Vars:  Private
    Integer(kind=StandardInteger) :: cKey, cKey_NL, nlKey
    Real(kind=DoubleReal), Dimension(1:3) :: xVect, yVect
    Integer(kind=StandardInteger) :: l, m, n, ghostCellKey
    Real(kind=DoubleReal), Dimension(1:27,1:3) :: ghostCellShift
    Real(kind=DoubleReal), Dimension(1:3) :: position, positionT
    Real(kind=DoubleReal) :: rdOld
!----------------------------------------------
! Optional arguments
!----------------------------------------------
    cKey_NL = 0
    ...
    Do l=-1,1
      Do m=-1,1
        Do n=-1,1
          ...
          ghostCellShift(ghostCellKey,2) = positionT(2)
          ghostCellShift(ghostCellKey,3) = positionT(3)
          !print *,l,m,n,ghostCellKey,ghostCellShift(ghostCellKey,1),&
          !   ghostCellShift(ghostCellKey,2),ghostCellShift(ghostCellKey,3)
        End Do
      End Do
    End Do
! Calculate volume
    volVec = MatMul(distortion,nl(cKey)%unitCellScaled)
    nl(cKey)%volumeDistorted = TripleProductSq(volVec)
! Loop through neighbour list
    Do nlKey = 1,nl(cKey)%length
! Ghost cell key
      ghostCellKey = (nl(cKey)%ghostCell(nlKey,1)+2)+&
                     (nl(cKey)%subCellWidth+1)*(nl(cKey)%ghostCell(nlKey,2)+1)+&
                     (nl(cKey)%subCellWidth+1)**2*(nl(cKey)%ghostCell(nlKey,3)+1)
! A and B position vectors
      xVect(1) = nl(cKey)%coordsMD(nl(cKey)%atomA_ID(nlKey),1)
      xVect(2) = nl(cKey)%coordsMD(nl(cKey)%atomA_ID(nlKey),2)
      xVect(3) = nl(cKey)%coordsMD(nl(cKey)%atomA_ID(nlKey),3)
      yVect(1) = nl(cKey)%coordsMD(nl(cKey)%atomB_ID(nlKey),1)+ghostCellShift(ghostCellKey,1)
      yVect(2) = nl(cKey)%coordsMD(nl(cKey)%atomB_ID(nlKey),2)+ghostCellShift(ghostCellKey,2)
      yVect(3) = nl(cKey)%coordsMD(nl(cKey)%atomB_ID(nlKey),3)+ghostCellShift(ghostCellKey,3)
      rdOld = nl(cKey)%rD(nlKey)
      nl(cKey)%rD(nlKey) = RdCoords(xVect, yVect, distortion)
      !print *,nlKey,rdOld,nl(cKey)%rD(nlKey)
      !print *,""
    End Do
!----------------------------------------------------------------------
      End If
    End Do
  End Subroutine distortNL
