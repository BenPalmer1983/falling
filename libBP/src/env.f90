...
! Define types
  ...
  Type :: oCmdArgs
    Integer(kind=StandardInteger) :: argCount
    Character(Len=64), Dimension(1:20) :: args
  End Type oCmdArgs
...
! Public variables
  ...
  Type(oCmdArgs) :: cmdArgs
! Make private
  Private
! Public
! ---- Variables
  Public :: envVars, cmdArgs
! ---- Functions
!  Public ::
! ---- Subroutines
  Public :: loadCmdArgs
...
!---------------------------------------------------------------------------------------------------------------------------------------
  ...
  Subroutine loadCmdArgs()
! Load command arguments into public type
    Implicit None   ! Force declaration of all variables
! Vars:  In/Out
! Vars:  Private
    Integer(kind=StandardInteger) :: n
    Character(Len=64) :: argTemp
! Clean array
    cmdArgs%args = WipeStringArray(cmdArgs%args)
! Argument count
    cmdArgs%argCount = command_argument_count()
! Load
    If(cmdArgs%argCount.gt.0)Then
      Do n=1,cmdArgs%argCount
        call get_command_argument(n, argTemp)
        cmdArgs%args(n) = trim(adjustl(argTemp))
      End Do
    End If
  End Subroutine loadCmdArgs
