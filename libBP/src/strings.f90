...
! ---- Functions
  ...
  Public :: BlankString         ! fill with spaces char(32)
  Public :: BlankStringArray
  Public :: WipeString          ! fill with \00    char(0)
  ...
  Public :: IsBlank
  Public :: ConcatStr
  Public :: JoinStr      ! join A and B overwriting trailing spaces or char(0), output size str A
  Public :: StrMatch
  Public :: TestBoolStr
! ---- Subroutines
  Public :: explode
...
!---------------------------------------------------------------------------------------------------------------------------------------
  Function StrMerge (strA, strB) RESULT (output)
! Merge string A over string B
    Implicit None  !Force declaration of all variables
! Vars:  In
    Character(*) :: strA
    Character(*) :: strB
! Vars:  Out
    Character(Len(strB)) :: output
! Vars:  Private
    Integer(kind=StandardInteger) :: i
! Loop through string
    Do i=1,len(strB)
      If(i.gt.len(strA))Then
        output(i:i) = strB(i:i)
      End If
      If((iChar(strA(i:i)).eq.0).or.(iChar(strA(i:i)).eq.32))Then
        output(i:i) = strB(i:i)
      Else
        output(i:i) = strA(i:i)
      End If
...
!---------------------------------------------------------------------------------------------------------------------------------------
  Function StrRmChunk (strIn, A, B, padCharIn) RESULT (output)
! Merge string A over string B
    Implicit None  !Force declaration of all variables
! Vars:  In
    Character(*) :: strIn
...
! Vars:  Out
    Character(Len(strIn)) :: output
! Vars:  Private
    Integer(kind=StandardInteger) :: i, j, strLen
...
! Loop through string
    ...
    j = 0
    Do i=1,strLen
      If((i.ge.A).and.(i.le.B))Then
        ! Skip
      Else
        j = j + 1
        output(j:j) = strIn(i:i)
      End If
    End Do
    If(j.lt.strLen)Then
      Do while(j.lt.strLen)
        j = j + 1
        output(j:j) = padChar(1:1)
      End Do
    End If
  End Function StrRmChunk
...
!
  ...
  Function IntToStrL (input, sizeStr) RESULT (output)
! Integer to string - leading zeros
    Implicit None  ! Force declaration of all variables
! Vars:  In
    Integer(kind=StandardInteger) :: input
    Integer(kind=StandardInteger) :: sizeStr
! Vars:  Out
    Character(Len=sizeStr) :: output
! Vars:  Private
    Integer(kind=StandardInteger) :: i, j, k, asciiVal
    Character(Len=16) :: tempRead
!
! Zero
    Do i=1,sizeStr
      output(i:i) = "0"
...
! Read in integer
    ...
    Do i=1,len(tempRead)
      ...
      If(k.lt.1)Then
        Exit
      Else
        asciiVal = iChar(tempRead(j:j))
        If((asciiVal.ge.48).and.(asciiVal.le.57))Then
          output(k:k) = tempRead(j:j)
        Else
          Exit
        End If
      End If
    End Do
  End Function IntToStrL
...
! Trim strings if required
  ...
  Function JoinStr(stringA, stringB) Result (stringC)
! Join two strings, returning in first string length
    Implicit None  ! Force declaration of all variables
! Vars:  In
    Character(*) :: stringA
...
! Init
    stringC = BlankString(stringC)
! Find "end" of string A
    j = Len(stringA)+1
    Do i=1,Len(stringA)
      j = j - 1
      If((stringA(j:j).eq." ").or.(ichar(stringA(j:j)).eq.0))Then
! do nothing
      Else
        Exit
      End If
...
! Join
    If(j.eq.0)Then
      stringC = stringB
    Else If(j.eq.Len(stringA))Then
      stringC = stringA
    Else
      stringC(1:j) = stringA(1:j)
      k = Len(stringA) - j
      If(k.gt.Len(stringB))Then
        k = Len(stringB)
      End If
      stringC(j+1:j+k) = stringB(1:k)
    End If
...
! Padding values
  ...
  Subroutine StrIntoStr (strIn, strOut)
! Write one string into another (blank) string
    Implicit None  !Force declaration of all variables
! Vars:  In/Out
    Character(*) :: strIn
    Character(*) :: strOut
...
! Str Len
    n = Len(strOut)
    If(Len(strIn).lt.n)Then
      n = Len(strIn)
    End If
    Do i=1,n
      strOut(i:i) = strIn(i:i)
    End Do
    If(i.lt.Len(strOut))Then
      Do j=i+1,Len(strOut)
        strOut(j:j) = " "
      End Do
    End If
  End Subroutine StrIntoStr
