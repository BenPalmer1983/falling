...
!---------------------------------------------------------------------------------------------------------------------------------------
  ...
  Function LMA(points, LMA_Call, parametersIn, weightingIn, limitsLowerIn, limitsUpperIn) &
...
! Vars:  In - Function call interface
    Interface
      Function LMA_Call(pSize, parameters, x)   ! Define the format of the call function
        ...
        Real(kind=DoubleReal) :: LMA_Call ! result
      End function LMA_Call
    End Interface
! Vars:  In
...
! ----------------
    Do n=1,20  ! maximum 20 loops
      rss = 0.0D0
      Do i=1,nPoints    ! Loop through data points (xi,yi) that function is being fit to
        x = points(i,1)
        y = points(i,2)
        fx = LMA_Call(size(parameters),parameters,x)
        R(i) = fx - y
        rss = rss + R(i)**2
        Do k=1,nParams
          ...
          fxP = LMA_Call(size(parametersT),parametersT,x)
          J(i,k) = (fxP-fx)/dVal
        End Do
      End Do
! Starting lambda
...
! calculate new parameters
        parameters = LMA_Calc(J,R,lambda,lambdaCutoff,parameters,weighting)
! Check for parameter limits
        ...
        End Do
! Calc RSS
        rssTrial = LMA_FunctionRSS(points, LMA_Call, parameters)
! Breakout if NaN
...
! Delayed gratification scheme - 1.5*lambda or 0.2*lambda
  ...
  Function LMA_Calc(J,R,lambda,lambdaCutoff, parametersIn,weighting) RESULT (parametersOut)
    Implicit None  !Force declaration of all variables
! In
...
! Prep covariance matrix
    ...
    End Do
! Matrix calculations
    JT = TransposeMatrix(J)     ! matrix.f90
    JTW = matmul(JT,W)
    JTWJ = matmul(JTW,J)
    JTWR = matmul(JTW,R)
    JTWJ_Diag = lambda*DiagMatrix(JTWJ) ! Dampening Matrix
    A = MatAdd(JTWJ,JTWJ_Diag)  ! matrix.f90
    B = -1.0D0*JTWR
    P = SolveLinearSet(A, B)  ! linearAlgebra.f90
! Update parameters
    Do i=1,size(P)
...
! Calculate starting lambda/lambda cutoff
    JT = TransposeMatrix(J)      ! matrix.f90
    JTJ = matmul(JT,J)
    JTJ_Inv = InvertMatrix(JTJ)  ! matrix.f90
! Check Marquardt parameter cutoff
    traceJTJ = Trace(JTJ_Inv)    ! matrix.f90
    lambda = traceJTJ**(-1)
  End Function LMA_Lambda

  Function LMA_FunctionRSS(points, LMA_Call, parameters) RESULT (rss)
! RSS between function and calculated points
    Implicit None  !Force declaration of all variables
! Vars:  In - Function call interface
    Interface
      Function LMA_Call(pSize, parameters, x)
        Import StandardInteger, DoubleReal
        Integer(kind=StandardInteger), Intent(IN) ::  pSize
        Real(kind=DoubleReal), Dimension(1:pSize), Intent(IN) :: parameters ! x vector
        Real(kind=DoubleReal), Intent(IN) :: x
        Real(kind=DoubleReal) :: LMA_Call ! result
      End function LMA_Call
    End Interface
...
! RSS
    ...
    Do i=1,size(points,1)
      x = points(i,1)
      y = points(i,2)
      fx = LMA_Call(size(parameters,1),parameters,x)
      rss = rss + (y - fx)**2
    End Do
  End Function LMA_FunctionRSS

!---------------------------------------------------------------------------------------------------------------------------------------
! Sample Functions for LMA
!---------------------------------------------------------------------------------------------------------------------------------------

  Function LMA_Quadratic(pSize,parameters,x) RESULT (y)
! Quadratic function
! Ax^2+Bx+C
! P(1) = A, P(2) = B, P(3) = C
    Implicit None  !Force declaration of all variables
    ...
    y = parameters(1)*x**2+parameters(2)*x+parameters(3)
  End Function LMA_Quadratic

  Function LMA_Polynomial(pSize,parameters,x) RESULT (y)
    Implicit None  !Force declaration of all variables
    Integer(kind=StandardInteger), Intent(IN) ::  pSize
    Real(kind=DoubleReal), Dimension(1:pSize), Intent(IN) :: parameters
    Real(kind=DoubleReal), Intent(IN) :: x
    Real(kind=DoubleReal) :: y
    y = CalcPolynomial(parameters, x)  ! calcFunctions.f90
  End Function LMA_Polynomial

  Function LMA_BirchMurn(pSize,parameters,x) RESULT (y)
    Implicit None  !Force declaration of all variables
    Integer(kind=StandardInteger), Intent(IN) ::  pSize
    Real(kind=DoubleReal), Dimension(1:pSize), Intent(IN) :: parameters
    Real(kind=DoubleReal), Intent(IN) :: x
    Real(kind=DoubleReal) :: y
    y = F_BirchMurn(x, parameters)  ! scienceFunctions.f90
  End Function LMA_BirchMurn

  Function LMA_Exp(pSize,parameters,x) RESULT (y)
    Implicit None  !Force declaration of all variables
    Integer(kind=StandardInteger), Intent(IN) ::  pSize
    Real(kind=DoubleReal), Dimension(1:pSize), Intent(IN) :: parameters
    Real(kind=DoubleReal), Intent(IN) :: x
    Real(kind=DoubleReal) :: y
    y = ExpCalc(x,parameters)  ! calcFunctions.f90
  End Function LMA_Exp

  Function LMA_ExpDens(pSize,parameters,x) RESULT (y)
! Density function  p(r) = ar^2 exp(br^2) + cr^2 exp(dr^2)
  ...
  Function LMA_Morse(pSize,parameters,x) RESULT (y)
...
! P(1) = D, P(2) = a, P(3) = rc
  ...
  Function LMA_MorseExtended(pSize,parameters,x) RESULT (y)
! Morse Function  V(r) = D((1-exp(a(rc-r)))^2-1)
! P(1) = D, P(2) = a, P(3) = rc
! not yet tested
    Implicit None  !Force declaration of all variables
...
! y = V(r)^2 + V'(r)^2
  ...
  Function LMA_LJ(pSize,parameters,x) RESULT (y)
