...
! Loop through pair potentials between atoms
      Do j=1,nl(cKey)%pairKeyArray(pKey,0)
        fN = nl(cKey)%pairKeyArray(pKey,j)
        If((nl(cKey)%rD(nlKey).le.potential%xMax(fN)).and.(nl(cKey)%rD(nlKey).ge.potential%xMin(fN)))Then
! Set search object
          searchObj%Fn = fN
...
! Store Pair Energy (Individual)
          nl(cKey)%atomEnergy(nl(cKey)%atomA_ID(nlKey),1) = &
            nl(cKey)%atomEnergy(nl(cKey)%atomA_ID(nlKey),1) + halfPairEnergy          ! Atom A pair energy (1)
          nl(cKey)%atomEnergy(nl(cKey)%atomB_ID(nlKey),1) = &
            nl(cKey)%atomEnergy(nl(cKey)%atomB_ID(nlKey),1) + halfPairEnergy          ! Atom B pair energy (1)
...
! Force on atom A
          nl(cKey)%forces(nl(cKey)%atomA_ID(nlKey),1) = nl(cKey)%forces(nl(cKey)%atomA_ID(nlKey),1) - forceArray(1)
          nl(cKey)%forces(nl(cKey)%atomA_ID(nlKey),2) = nl(cKey)%forces(nl(cKey)%atomA_ID(nlKey),2) - forceArray(2)
          nl(cKey)%forces(nl(cKey)%atomA_ID(nlKey),3) = nl(cKey)%forces(nl(cKey)%atomA_ID(nlKey),3) - forceArray(3)
! Force on atom B
          nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),1) = nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),1) + forceArray(1)
          nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),2) = nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),2) + forceArray(2)
          nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),3) = nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),3) + forceArray(3)
        End If
      End Do
!-------------------------------------------------------------------------------
...
! Loop through pair potentials between atoms
      Do j=1,nl(cKey)%densityKeyArray(dKey,0,1)
        Fn = nl(cKey)%densityKeyArray(dKey,j,1)
        If((nl(cKey)%rD(nlKey).le.potential%xMax(fN)).and.(nl(cKey)%rD(nlKey).ge.potential%xMin(fN)))Then
! Set search object
          searchObj%Fn = Fn
          searchObj%x = nl(cKey)%rD(nlKey)
...
! Loop through embedding energies for atom
      Do j=1,nl(cKey)%embeddingKeyArray(eKey,0,1)
        Fn = nl(cKey)%embeddingKeyArray(eKey,j,1)
        If((nl(cKey)%electronDensity(cKey_Loop,1).le.potential%xMax(fN)).and.&
        (nl(cKey)%electronDensity(cKey_Loop,1).ge.potential%xMin(fN)))Then
! Set search object
          searchObj%Fn = Fn
...
! Force on atom B
      nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),1) = &
        nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),1) - forceArray(1)
      nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),2) = &
        nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),2) - forceArray(2)
      nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),3) = &
        nl(cKey)%forces(nl(cKey)%atomB_ID(nlKey),3) - forceArray(3)
    End Do
!-------------------------------------------------------------------------------
!
!
