...
! Interfaces
  ...
  End Interface Trace
  Interface MatAdd
    Module Procedure MatAdd1D_D, MatAdd2D_D
  End Interface MatAdd
...
! Trace
  ...
  Function MatAdd1D_D(xMatrix,yMatrix) RESULT (oMatrix)
! Force declaration of all variables
    Implicit None
! Declare variables
    Integer(kind=StandardInteger) :: i
    Real(kind=DoubleReal), Dimension(:) :: xMatrix, yMatrix
    Real(kind=DoubleReal), Dimension(1:size(xMatrix,1)) :: oMatrix
! Initialise variables
    oMatrix = 0.0D0
! Add matrices
    If(size(xMatrix,1).eq.size(yMatrix,1))Then
      Do i=1,size(xMatrix,1)
        oMatrix(i) = xMatrix(i) + yMatrix(i)
      End Do
    End If
  End Function MatAdd1D_D

  Function MatAdd2D_D(xMatrix,yMatrix) RESULT (oMatrix)
! Force declaration of all variables
    Implicit None
! Declare variables
...
! Multiply matrices
  ...
  Function MatMult1D(xMatrix,yMatrix) RESULT (oScalar)
! Force declaration of all variables
    Implicit None
! Declare variables
    Integer(kind=StandardInteger) :: i,xRows,yRows
    Real(kind=DoubleReal), Dimension(:) :: xMatrix, yMatrix
    Real(kind=DoubleReal) :: oScalar
! matrix(row,column)
! Initialise variables
    oScalar = 0.0D0
    xRows = size(xMatrix,1)
    yRows = size(yMatrix,1)
! Multiply matrices
    If(xRows.eq.yRows)Then
      Do i=1,xRows
        oScalar = oScalar + xMatrix(i) * yMatrix(i)
      End Do
    End If
  End Function MatMult1D
      ...
      Do while(sortFlag)
        ...
        Do row=1,(size(xMatrix,1)-1)
          If(xMatrix(row).lt.xMatrix(row+1))Then
            sortFlag = .true.
            Call swapRows(xMatrix,row,row+1)  ! general.f90
            Call swapRows(pivotMap,row,row+1) ! general.f90
          End If
        End Do
      End Do
    End If
    ...
    If(operation.eq."A".or.operation.eq."R")Then
      ...
      Do row=1,size(xMatrix,1)
        If(swapFlags(row))Then
          ...
          Call swapRows(xMatrix,row,pivotMap(row))  ! general.f90
        End If
      End Do
    End If
      ...
      Do while(sortFlag)
        ...
        Do row=1,(size(xMatrix,1)-1)
          If(sortMatrix(row).lt.sortMatrix(row+1))Then
            sortFlag = .true.
            Call swapRows(xMatrix,row,row+1)     ! general.f90
            Call swapRows(sortMatrix,row,row+1)  ! general.f90
            Call swapRows(pivotMap,row,row+1)    ! general.f90
          End If
        End Do
      End Do
