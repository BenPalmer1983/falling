...
! Declare variables
  ...
  Function Quadratic(a,b,c,r_In) Result (result)
! Quadratic function
    Implicit None ! Force declaration of all variables
! Vars:  In
    Real(kind=DoubleReal) :: a, b, c
    Integer(kind=StandardInteger), Optional :: r_In
! Vars:  Out
    Real(kind=DoubleReal) :: result
! Vars:  Private
    Integer(kind=StandardInteger) :: r
! Optional arguments
    r = 1
...
! Choose result
    If(r.eq.1)Then
      result = (-1.0D0*b+sqrt(b*b-4.0D0*a*c))/(2.0D0*a)
    Else
      result = (-1.0D0*b-sqrt(b*b-4.0D0*a*c))/(2.0D0*a)
    End If
