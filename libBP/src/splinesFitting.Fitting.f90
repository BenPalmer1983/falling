...
! Store results
  ...
  Function BestFitLine(dataPoints) RESULT (parameters)
! fits a straight line to points
    Implicit None   ! Force declaration of all variables
! Vars:  In
    Real(kind=DoubleReal), Dimension(:,:) :: dataPoints
! Vars:  Out
...
! Vars:  Private
    Integer(kind=StandardInteger) :: i, n
    Real(kind=DoubleReal) :: meanX, meanY
...
! Calc mean x and y
    n = size(dataPoints,1)
    Do i=1,n
      meanX = meanX + dataPoints(i,1)
      meanY = meanY + dataPoints(i,2)
    End Do
    meanX = meanX / n
    meanY = meanY / n
    Do i=1,n
      numerator = numerator + (dataPoints(i,1)-meanX)*(dataPoints(i,2)-meanY)
      denominator = denominator + (dataPoints(i,1)-meanX)**2
    End Do
    parameters(2) = numerator/denominator         ! Slope
    parameters(1) = meanY-parameters(2)*meanX     ! Intercept
  End Function BestFitLine

  Function BestFitExp(dataPoints) RESULT (parameters)
! straight line fit, to fit single exponential a exp(bx)
! y(x) = a exp(bx)
! ln(y(x)) = ln(a) + b x
    Implicit None   ! Force declaration of all variables
! Vars:  In
...
! Vars:  Out
    Real(kind=DoubleReal), Dimension(1:2) :: parameters
! Vars:  Private
    Real(kind=DoubleReal), Dimension(1:size(dataPoints,1),1:size(dataPoints,2)) :: dataPointsLn
    Integer(kind=StandardInteger) :: i
! Fill ln data points
    Do i=1,size(dataPoints,1)
      dataPointsLn(i,1) = dataPoints(i,1)
      dataPointsLn(i,2) = log(dataPoints(i,2))
    End Do
    parameters = BestFitLine(dataPointsLn)
    parameters(1) = exp(parameters(1))
  End Function BestFitExp

  Function BestFitExpSq(dataPoints) RESULT (parameters)
! straight line fit, to fit single exponential a exp(bx)
! y(x) = a exp(bx)^2
! ln(sqrt(y(x))) = ln(sqrt(a)) + b x
    Implicit None   ! Force declaration of all variables
! Vars:  In
    Real(kind=DoubleReal), Dimension(:,:) :: dataPoints
! Vars:  Out
...
! Fill ln data points
    Do i=1,size(dataPoints,1)
      dataPointsLn(i,1) = dataPoints(i,1)
      dataPointsLn(i,2) = log(sqrt(dataPoints(i,2)))
    End Do
    parameters = BestFitLine(dataPointsLn)
    parameters(1) = exp(parameters(1))**2
  End Function BestFitExpSq


  Function MorseFit(dataPoints) RESULT (parameters)
! Fit Morse function to data
!  parameters(1) = De
!  parameters(2) = a
!  parameters(3) = rC
    Implicit None   ! Force declaration of all variables
...
! Vars:  Private
    ...
    parameters(3) = 3.5D0
! LMA fit
    parameters = LMA(dataPoints, LMA_Morse, parameters)
  End Function MorseFit


  Function MorseFit_Extended(dataPoints, dataPointsDYDX) RESULT (parameters)
! Fit Morse function to data x,y and x,y'
!  parameters(1) = De
!  parameters(2) = a
!  parameters(3) = rC
    Implicit None   ! Force declaration of all variables
! Vars:  In
    Real(kind=DoubleReal), Dimension(:,:) :: dataPoints
    Real(kind=DoubleReal), Dimension(:,:) :: dataPointsDYDX
! Vars:  Out
    Real(kind=DoubleReal), Dimension(1:3) :: parameters
! Vars:  Private
    Integer(kind=StandardInteger) :: i
    Real(kind=DoubleReal), Dimension(1:size(dataPoints,1),1:size(dataPoints,2)) :: dataPointsNew
! Combine x,y and x,y' data points
    Do i=1,size(dataPoints,1)
      dataPointsNew(i,2) = dataPoints(i,2)**2 + dataPointsDYDX(i,2)**2
    End Do
! Starting parameters
    parameters = MorseFit(dataPoints)
! LMA fit
    parameters = LMA(dataPointsNew, LMA_MorseExtended, parameters)
  End Function MorseFit_Extended


  Function LJFit(dataPoints) RESULT (parameters)
! Fit Morse function to data
!  parameters(1) = De
!  parameters(2) = a
!  parameters(3) = rC
...
! Vars:  Private
    ...
    parameters(2) = 1.0D0
! LMA fit
    parameters = LMA(dataPoints, LMA_LJ, parameters)
  End Function LJFit
...
! Bulk Modulus Fit
    If(calcFunctionT(1:3).eq."BM1")Then   ! BM Restrict BP0
      ...
      Do i=1,pointsOutCount
        dataPointsOut(i,1) = xStart+(i-1)*xInc
        dataPointsOut(i,2) = F_BirchMurn(dataPointsOut(i,1),parameters4)
      End Do
    End If
