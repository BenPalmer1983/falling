...
! Get file info
    potFile = fileInfo(filePath)
    !Call printFileInfo(potFile)
! Read in file
...
! Tabulated Data File
      If(tempField(1:9).eq."#TAB_FILE")Then
        filePathTemp = trim(fieldArray(2))
        If(filePathTemp(1:1).eq."/")Then    ! If absolute path already specified
          potential%tabFilePath(fN) = filePathTemp
        Else                                ! If relative, make it relative to main pot file
          potential%tabFilePath(fN) = trim(potFile%dir)//"/"//trim(filePathTemp)
        End If
      End If
! ZBL Parameters
