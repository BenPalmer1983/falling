...
! ---- Subroutines
  Public :: sortArray
  Public :: sortArray_DP_2D
! Interfaces
  Interface sortArray
    Module Procedure sortArray_DP_1D, sortArray_DP_2D, sortArray_Int_1D, sortArray_Int_2D
...
! INT 1D
! ------------------
  Subroutine sortArray_Int_2D(arrayIn,orderIn,startRowIn,endRowIn)
! Sort array
!
    Implicit None   ! Force declaration of all variables
! Vars:  In/Out
...
! Sort array: Ascending
    If(order.eq."ASC")Then
      ...
      Do While(keepLooping)
        keepLooping = .false.
        Do i=startRow,endRow-1
          If(arrayIn(i,1).gt.arrayIn(i+1,1))Then
            keepLooping = .true.
            Call swapRows(arrayIn,i,i+1)
          End If
        End Do
    ...
    Else
      ...
      Do While(keepLooping)
        keepLooping = .false.
        Do i=startRow,endRow-1
          If(arrayIn(i,1).lt.arrayIn(i+1,1))Then
            keepLooping = .true.
            Call swapRows(arrayIn,i,i+1)
          End If
        End Do
      End Do
    End If
  End Subroutine sortArray_Int_2D
